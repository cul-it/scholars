...
import edu.cornell.library.scholars.webapp.utils.sparqlrunner.SparqlQueryRunner.ExecutingSelectQueryContext;
import edu.cornell.library.scholars.webapp.utils.sparqlrunner.SparqlQueryRunner.SelectQueryContext;
...
class RdfServiceSelectQueryContext implements SelectQueryContext {
    ...
    @Override
    public RdfServiceSelectQueryContext bindVariableToLiteral(String name,
            Literal literal) {
        return new RdfServiceSelectQueryContext(rdfService,
    ...
    private static class RdfServiceExecutingQueryContext
            implements ExecutingSelectQueryContext {
        private final RDFService rdfService;
        private final QueryHolder query;
        ...
        public StringResultsMapping toStringFields(String... names) {
            ...
            try {
                ResultSet results = RDFServiceUtils
                        .sparqlSelectQuery(query.getQueryString(), rdfService);
        ...
        public void writeToOutput(OutputStream output) {
            try {
                InputStream resultStream = rdfService
                        .sparqlSelectQuery(query.getQueryString(), JSON);
