...
<#macro renderCode display="icon" width="125">
    <#if hasValidVCard()>
        ...
        <#local qrCodeLinkedImage>
            <a title="Export QR codes" href="${qrData.exportQrCodeUrl}" title="QR Code"><@qrCodeVCard qrCodeWidth=width /></a>
        </#local>
        <#if (display == "full")>
            <h5 class="qrCode">vCard QR</h5>
            ${qrCodeLinkedImage}
        <#elseif (display == "icon")>
                <a id="qrIcon" title="vCard QR Code" href="${qrData.exportQrCodeUrl}" title="QR Code"><img class="middle" style="vertical-align:top;" src="${urls.images}/individual/qr-code-icon.png" alt="qr icon" /></a>
