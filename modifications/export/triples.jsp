...
create table stmt (subj text, pred text, obj text);
<%
     ...
     // iterate through to find all the statements in the model for this property
    	for (Iterator iter = model.listStatements((Resource)null,p,(RDFNode)null); iter.hasNext();) {
    	    ...
    	    if (obj.isResource()) {
    	    	...
    	    	value = lit.getLexicalForm().trim();
    	    	Tidy tidy = new Tidy();
    	    	value = tidy. parse(new InputStream(value),value);
    	    }%>
insert into stmt (subj,pred,obj) values ('<%=org.apache.commons.lang.StringEscapeUtils.escapeSql(subj)%>',
'<%=org.apache.commons.lang.StringEscapeUtils.escapeSql(p.getURI())%>',
'<%=org.apache.commons.lang.StringEscapeUtils.escapeSql(value)%>');<%
		}
	}
}
