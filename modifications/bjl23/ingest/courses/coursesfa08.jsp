...
<%! 
	String RAW_DATA_MODEL = "Class Data Non-AG 2008FA";
...
<%!
	...
	Property meetingPatternProperty = ResourceFactory.createProperty(tboxNs+"course_MeetingPatternSdescr");
	Property startAndEndTimeProperty = ResourceFactory.createProperty(tboxNs+"course_StartandEndTime");
	Property componentProperty = ResourceFactory.createProperty(tboxNs+"course_SsrComponent");
	Property facilityProperty = ResourceFactory.createProperty(tboxNs+"course_FacilityLdescr");	
	Property hasSubCourseProperty = ResourceFactory.createProperty(tboxNs+"hasSubCourse");
	Property classSectionProperty = ResourceFactory.createProperty(tboxNs+"course_ClassSection");
	...
	Property subjectProperty = ResourceFactory.createProperty(tboxNs+"course_Subject");
	Property catalogNumberProperty = ResourceFactory.createProperty(tboxNs+"course_CatalogNbr");
	Property courseIdProperty = ResourceFactory.createProperty(tboxNs+"course_CourseId");
	Property gradingBasisProperty = ResourceFactory.createProperty(tboxNs+"course_GradingBasis");
	Property unitsMinimumProperty = ResourceFactory.createProperty(tboxNs+"course_UnitsMinimum");
	Property unitsMaximumProperty = ResourceFactory.createProperty(tboxNs+"course_UnitsMaximum");	
	Property courseTitleLong = ResourceFactory.createProperty(tboxNs+"course_CourseTitleLong");
	Property academicTermProperty = ResourceFactory.createProperty(tboxNs+"course_Academic_Term_Ldescr");

	Property vivoClassCreditsProperty = ResourceFactory.createProperty(tboxNs+"http://vivo.library.cornell.edu/ns/0.1#classCredits");
...
<%
	...
	"} WHERE { \n" +
	"    ?course courses:course_PrincipalorSubcourse \"Principal\" \n" +
	"}\n" ;
	...
	"} WHERE { \n" +
	"    ?course courses:course_PrincipalorSubcourse \"Sub-courses\" \n" +
	"}\n" ;
	...
	"    ?course courses:course_CourseId ?id . \n" +
	"    ?subCourse rdf:type courses:SubCourse . \n" +
	"    ?subCourse courses:course_CourseId ?id \n" +
	"}\n" ;
	...
	for(int i=0; i<typeURIs.length; i++) {
		...
		"} WHERE { \n" +
		"    ?course courses:course_SsrComponent \""+values[i]+"\" \n" +
		"}\n" ;
	...
	//Okay, now we're going to iterate through the principal courses and link them to instructors
	...
	Model tmp = ModelFactory.createDefaultModel();
	for (Iterator principalCoursesIt = workOntModel.listIndividuals(ResourceFactory.createResource(principalCourseClassURI)); principalCoursesIt.hasNext(); ) {
		Individual principalCourse = (Individual) principalCoursesIt.next();
		RDFNode valueNode = principalCourse.getPropertyValue(ResourceFactory.createProperty(tboxNs+"course_PrimaryInstructor"));
		if (valueNode != null) {
