...
public class SparqlQueryRunnerTest extends AbstractTestClass {
    ...
    private static final JsonObject EXPECTED_SELECT_RESULTS = JSON
            .parse("" + "{  " //
                    + "  'head' : {  " //
                    + "    'vars' : [ 's', 'p', 'o' ]  " //
                    + "  } , " //
    ...
    private static final String ASK_QUERY_TRUE = "ASK { <" + SUBJECT_URI + "> <"
            + PREDICATE_URI + "> \"" + OBJECT_VALUE + "\" }";
    private static final String ASK_QUERY_FALSE = "ASK { <" + SUBJECT_URI
            + "> <" + PREDICATE_URI + "> \"" + OBJECT_VALUE + "XXX\" }";
    ...
    @Test
    public void askQueryTrueAgainstModel() {
        assertTrue(createAskQueryContext(model, ASK_QUERY_TRUE).execute()
                .toBoolean());
    }

    @Test
    public void askQueryFalseAgainstModel() {
        assertFalse(createAskQueryContext(model, ASK_QUERY_FALSE).execute()
                .toBoolean());
    }

    @Test
    public void askQueryTrueAgainstRDFService() {
        assertTrue(createAskQueryContext(rdfService, ASK_QUERY_TRUE).execute()
                .toBoolean());
    }

    @Test
    public void askQueryFalseAgainstRDFService() {
        assertFalse(createAskQueryContext(rdfService, ASK_QUERY_FALSE).execute()
                .toBoolean());
    }
    ...
    private void assertExpectedConstructResults() {
        assertEquals(model.listStatements().toSet(),
                constructed.listStatements().toSet());
    }
}
