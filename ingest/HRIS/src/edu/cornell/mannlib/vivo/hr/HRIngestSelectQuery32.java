...
public class HRIngestSelectQuery32 {
    ...
    static final Logger logger = Logger.getLogger(HRIngestSelectQuery32.class);
		...
		try {  // begin try for gathering new person RDF
			...
			while (iter.hasNext(  )) {
				...
				try {  // with emplId in query, create CONSTRUCT rdf and push to HRIS model 
					...
					ResourceUtils.renameResource( (Resource) iter, resourceNewName);
