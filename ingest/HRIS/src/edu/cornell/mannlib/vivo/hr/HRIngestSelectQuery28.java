...
public class HRIngestSelectQuery28 {
	...
	public static void main(String[] args) throws Exception {
	    ...
	    Logger logger = Logger.getLogger(HRIngestSelectQuery28.class);
		...
		logger.info("constructed model from query, writing...");                   
		WriteRdf(HRISMatchFileName, mdlHRISMatch, "N-TRIPLE");
		logger.info("created model named mdlHRISmatch and wrote to file.");
		...
		logger.info("creating model with HRIS emplIds and VIVO uris...");
		Model mdlHRISVIVO = MakeNewModelCONSTRUCT(qStrHRISinVIVO);                 
		WriteRdf(HRISVIVOFileName, mdlHRISVIVO, "N-TRIPLE");
		logger.info("created model named mdlHRISinVIVO.");
		...
		logger.info("creating model with ALL HRIS emplIds...");
		Model mdlAllHRIS = MakeNewModelCONSTRUCT(qStrAllHRIS);                                
		WriteRdf(allHRISFileName, mdlAllHRIS, "N-TRIPLE");
		logger.info("querying HRISsource and populating Model mdlAllHRIS...");
		...
		logger.info("read in Model mdlHRISMatch from file hrisMatchEmplId.nt...");
		 */
		...
		logger.info("mdlHRISDiff now contains emplIds for HRIS not in VIVO .");
		...
		try {  // begin try for gathering new person RDF
			...
			while (iter.hasNext(  )) {
				...
				try {  // with emplId in query, create CONSTRUCT rdf and push to HRIS model 
					...
					logger.info("generating RDF for " + NewPersonId + " , adding to output model");
					...
					logger.info("finding positions for " + NewPersonId + "...");
					...
					try {
						...
						logger.info("analyzing statements in person RDF...");
					...
					try {  // write N3 to console and append to output model
						...
						logger.info("found position for " + NewPersonId + " , adding to output model");
						mdlAllNewPositionList.add(mdlNewPositionList);
						logger.info("done writing to position output model");

					} catch (Exception e)   {
						logger.info("exception writing New Person RDF!  Error" + e);
					} finally {
						logger.info("done processing " + NewPersonId + ".");
					}

				} catch (Exception e)   {
					logger.info("exception while creating the output model!  Error" + e);
				} finally {
			...
			logger.info("exception creating RDF for new people...  Error" + e);
		} finally {
			// wrap it up
		}
		// future methods: 
