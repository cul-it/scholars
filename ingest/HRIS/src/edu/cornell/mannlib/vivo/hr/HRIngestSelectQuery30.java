...
public class HRIngestSelectQuery30 {
	...
	public static void main(String[] args) throws Exception {
	    ...
	    Logger logger = Logger.getLogger(HRIngestSelectQuery30.class);
		...
		try {  // begin try for gathering new person RDF
			...
			try {
				...
				while (posniter.hasNext(  )) {
					...
					//remove when working
					System.out.println(qStrNewPositionRDF);
				...
				logger.info("skipped orglist query");
				logger.debug(" .");
			...
			try {
				// list the statements in the position list Model (each individual position ID)
				ResIterator orgiter = mdlAllNewOrgList.listSubjects(  );
				while (orgiter.hasNext(  )) {
					String NewOrgId = orgiter.next(  ).toString(  );

					//with every position Id in the list, generate appropriate RDF
					logger.debug("constructing RDF for " + NewOrgId + "...");
					String[] strs4 = {"qStrNewOrgRDFa.txt", NewOrgId, "qStrNewOrgRDFb.txt"};
					String qStrNewOrgRDF = ConcatQueryString(strs4); 
					...
					logger.debug("done processing " + NewOrgId + ".");
				...
				String allOrgFileName = fileRDFPath + "allOrgList.nt";
				WriteRdf(allOrgFileName, mdlAllNewOrgList, "N-TRIPLE");
				logger.debug("did orglist query");
				logger.debug(" .");
			...
			logger.error("exception creating RDF for new people...  Error" + e);
		} finally {
			// wrap it up
		}
		// future methods: 
